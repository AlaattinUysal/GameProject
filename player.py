    ...
    def update_animation(self, arrow_group=None):
        ...
        if self.is_collecting_potion:
            ...
            if self.potion_update_counter >= self.potion_animation_speed:
                ...
                if self.potion_frame_index >= len(self.idle_frames):
                    self.is_collecting_potion = False
                    self.potion_frame_index = 0
                    self.frame_index = 0
                else:
                    self.image = self.idle_frames[self.potion_frame_index]
        ...
        # Boşta animasyonu
            ...
            frames = self.idle_frames if not self.is_collecting_potion else self.idle_frames
            self.frame_index = (self.frame_index + 1) % len(frames)
            self.image = frames[self.frame_index]
