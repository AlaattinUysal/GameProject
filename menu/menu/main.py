...
class Menu:
    ...
    def start_game(self):
        ...
        import sys
        import os
        # Geçerli dizini al (menu/menu dizini)
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # Ana proje dizini
        project_root = os.path.abspath(os.path.join(current_dir, '..', '..'))
        game_path = os.path.join(project_root, 'game.py')
        # Python yorumlayıcısını kullanarak game.py'yi çalıştır, cwd'yi ana dizin yap
        subprocess.run([sys.executable, game_path], cwd=project_root)
        sys.exit()
